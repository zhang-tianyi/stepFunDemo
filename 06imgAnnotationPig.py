...
def main():
    input_image_path = "./img/pigs/猪1.jpeg"  # 替换为你的输入图像路径
    model_name = "step-1v-8k"  # 定义使用的模型名称，确保模型名称正确
    max_retries = 5
    attempt = 0

    # 检查输入图像路径是否存在
    if not os.path.isfile(input_image_path):
        print(f"输入图像文件不存在: {input_image_path}")
        return
    ...
    while attempt < max_retries:
        try:
            analysis_result = analyze_image(input_image_path, model_name, image_size)
            ...
            # {
            #     "pigs": [
            #         {"id": "pig1", "position": [100, 150, 200, 250]},
            #         {"id": "pig2", "position": [300, 400, 400, 500]},
            #         ...
            #     ]
            # }
            ...
            if validate_coordinates(pigs, image_size):
                ...
                output_image_path = os.path.join(os.path.dirname(input_image_path), "output", output_filename)

                annotate_image(input_image_path, pigs, output_image_path)
